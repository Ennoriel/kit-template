import { formDataToObject, isHexColorValid } from 'chyme';
...
export const GET: RequestHandler = ({ params, url }) => {
	...
	const bg = query.bg ?? false;
	const size = params.size;

	const sSvg = `
		<svg viewBox="0 0 150 150" height="150" width="150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			${bg && `<rect width="150" height="150" x="0" y="0" fill="${color}"></rect>`}
			<rect width="140" height="140" x="5" y="5" fill="none" stroke="${
				bg ? 'white' : color
			}" stroke-width="10"></rect>
			<text x="30" y="100" font-size="75" fill="${
				bg ? 'white' : color
			}" font-weight="900" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen-Sans, Ubuntu, Cantarell, 'Helvetica Neue', sans-serif">FC</text>
		</svg>
	`;
	...
	const mSvg = `
		<svg viewBox="0 0 450 150" height="150" width="450" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="user-select=none;">
			${bg && `<rect width="450" height="150" x="0" y="0" fill="${color}"></rect>`}
			<g fill="${
				bg ? 'white' : color
			}" font-weight="600" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen-Sans, Ubuntu, Cantarell, 'Helvetica Neue', sans-serif">
			...
			<path d="M 60 135 L 15 135 L 15 15 L 435 15 L 435 135 L 380 135"
				stroke="${bg ? 'white' : color}" stroke-width="3" fill="none" />
		</svg>
	`;
